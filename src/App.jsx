...
const BakeryApp = () => {
  ...
  // Custom router state: 'store' for storefront, 'admin' for Khushi's Console
  ...
  // Updated Checkout Logic: Correctly receives and formats the full dynamic message
  const handleCheckout = useCallback((type, deliveryDate, address, fullOrderSummary) => {
    const khushiNumber = "919136371662"; 

    if (type === 'whatsapp') {
      let finalMessage = fullOrderSummary;
      
      // Fallback message composition if the string is empty
      if (!finalMessage) {
        const itemSummary = cartItems.map(i => `• ${i.name} (x${i.qty}) = ₹${i.price * i.qty}`).join('\n');
        finalMessage = `Hi Khushi! I'd like to place an order...\n\n${itemSummary}\n\nTotal: ${formattedTotal}\nDelivery Date: ${deliveryDate}\nAddress: ${address}`;
      }

      window.open(`https://wa.me/${khushiNumber}?text=${encodeURIComponent(finalMessage)}`, '_blank');
    } else if (type === 'call') {
      window.open(`tel:+${khushiNumber}`, '_self');
    }
    ...
    <div className="relative w-full min-h-screen bg-[#080808] text-white overflow-x-hidden">
      ...
      {!isLoading && (
        <div className="animate-in fade-in duration-1000">
          {view === 'store' ? (
            <>
              {/* Dynamic Customer Drawer Overlay */}
              <Cart 
                isOpen={isCartOpen} 
              ...
              {/* Added onAdminClick callback to transition to dashboard securely */}
              <Navbar 
                cartCount={cartCount} 
              ...
              {/* 3. SMOOTH MOMENTUM SCROLL LAYOUT */}
            ...
            /* Secure Baker Console view */
            <ClientDashboard onBackToStore={() => setView('store')} />
          )}
        </div>
      )}
    </div>

...
const Navbar = ({ cartCount, onOpenCart }) => {
  ...
  // POWERFUL FIX: Force scroll and reset state instantly
  const handleLogoClick = (e) => {
    e.preventDefault();
    e.stopPropagation(); // Stop the event from bubbling up

    // Force a scroll to the actual top of the document
    window.scrollTo({
      top: 0,
      left: 0,
      behavior: 'smooth'
  ...
  return (
    <nav className="w-full bg-white/95 backdrop-blur-md border-b border-black/5 z-[100] relative">
      <div className="px-4 py-3 sm:px-12 sm:py-6 max-w-[1600px] mx-auto flex items-center justify-between">
        ...
        {/* LOGO SECTION: Changed to Button for better click reliability */}
        <button 
        ...
        >
          <img 
            ...
            className="w-8 h-8 sm:w-12 sm:h-12 rounded-full object-cover shadow-sm border border-black/5 transition-transform group-hover:scale-105 pointer-events-none" 
          />
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          >
            ...
            <span className="text-[10px] sm:text-[12px] font-black uppercase tracking-tighter sm:tracking-widest pointer-events-none">
              Bag
            </span>
          </button>

...
const Navbar = ({ cartCount, onOpenCart }) => {
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-[100] transition-all duration-300">
      {/* 1. BIGGER 24-HOUR NOTICE BANNER */}
      ...
      >
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-10">
            {navLinks.map((link) => (
              <a 
                key={link.name}
                href={link.href} 
                className="text-[11px] font-bold uppercase tracking-[0.2em] text-slate-600 hover:text-[#E89EB8] transition-colors relative group"
              >
                {link.name}
                <span className="absolute -bottom-1 left-0 w-0 h-[2px] bg-[#E89EB8] transition-all duration-300 group-hover:w-full" />
              </a>
            ))}
          </div>
          ...
          <div className="flex-1 flex justify-end items-center gap-6">
            <button 
              onClick={onOpenCart}
              className="relative p-2 text-slate-900 hover:text-[#E89EB8] transition-colors group"
            >
              ...
              {cartCount > 0 && (
                <motion.span 
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
                  className="absolute -top-1 -right-1 bg-[#E89EB8] text-white text-[9px] font-black w-5 h-5 rounded-full flex items-center justify-center shadow-sm"
                >
                  {cartCount}
                </motion.span>
              )}
            ...
            >
              <div className={`h-[2px] bg-slate-900 transition-all ${isMenuOpen ? 'w-6 rotate-45 translate-y-2' : 'w-5'}`} />
              <div className={`h-[2px] bg-slate-900 w-6 ${isMenuOpen ? 'opacity-0' : 'opacity-100'}`} />
              <div className={`h-[2px] bg-slate-900 transition-all ${isMenuOpen ? 'w-6 -rotate-45 -translate-y-2' : 'w-4'}`} />
            </button>
          </div>
        </div>
